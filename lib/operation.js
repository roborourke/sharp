...
/**
 * Map a look up table (LUT) to the image from an index image and lut image
 * @param {Buffer|String} lut - Buffer containing image data or String containing the path to an image file.
 * @returns {Sharp}
 * @throws {Error} Invalid parameters
 */
function maplut (lut) {
  if (!is.defined(lut)) {
    throw new Error('Missing lut image');
  }

  this.options.withLut = true;
  this.options.lut = this._createInputDescriptor(lut);

 ...
 * By default output image will be web-friendly sRGB, with additional channels interpreted as alpha channels.
 * @param {String} [profile] - path to custom icc profile
 * @returns {Sharp}
 * @throws {Error} Invalid parameters
 */
function withIcc (profile) {
  if (!is.string(profile)) {
    throw new Error('Invalid profile path ' + profile);
  }
  this.options.iccTransformPath = path.resolve(__dirname, profile);
  return this;
}
